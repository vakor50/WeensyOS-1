...
// finds next empty process and tries to create a thread
// not a working implementation
static pid_t do_newthread(process_t *old_proc, int start_function)
{
    pid_t p = 1;
    process_t *new_thread = NULL;
    int broke = 0;
    for (p = 1; p < NPROCS; p++)
      {
    ...
    if (broke != 1) // the for loop never found an empty process
      return -1; // error

    // initialize all the components of the new thread
    new_thread->p_registers = old_proc->p_registers;
    new_thread->p_registers.reg_eip = start_function;
    new_thread->p_state = P_RUNNABLE;
    new_thread->p_registers.reg_eax = 0;
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if (proc_array[current->p_is_waiting].p_state == P_BLOCKED)
		  {
		    proc_array[current->p_is_waiting].p_registers.reg_eax = current->p_exit_status;
		    proc_array[current->p_is_waiting].p_state = P_RUNNABLE;
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
		    ...
		    || proc_array[p].p_is_waiting != current->p_pid)
			current->p_registers.reg_eax = -1;
		else if (proc_array[p].p_state == P_ZOMBIE)
		  {
			current->p_registers.reg_eax = proc_array[p].p_exit_status;
			proc_array[p].p_state = P_EMPTY;
			proc_array[p].p_is_waiting = 0;
		  }
		else
		  {
	...
	case INT_SYS_NEWTHREAD:
	  {
		...
		// address of start_function
		run(current);      
	  } 
	case INT_SYS_KILL:
	  {
		pid_t to_end;
		to_end = current->p_registers.reg_eax;
		if (proc_array[to_end].p_state == P_EMPTY 
		    || to_end <= 0 
		    || to_end >= NPROCS 
		    || to_end == current->p_pid)
		  current->p_registers.reg_eax = -1; // can't kill this proc
		pid_t p = current->p_registers.reg_eax;
		if (p <= 0 || p >= NPROCS || p == current->p_pid
		    || proc_array[p].p_state == P_EMPTY
		    || proc_array[p].p_is_waiting != current->p_pid)
...
do_fork(process_t *parent)
{
  ...
  do
    ...
    if (!new_proc)
      return -1;
  // set new registers to same as parent
  ...
  new_proc->p_is_waiting = parent->p_pid;
  return p;
}
